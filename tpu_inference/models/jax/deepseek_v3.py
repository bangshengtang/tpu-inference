...
class DeepseekV3BaseAttention(nnx.Module):
    ...
    def setup_specific_layers(self, *args):
        pass

    def compute_q_projection(self, *args):
        raise NotImplementedError

    def compute_kv_projection(self, *args):
        raise NotImplementedError

    def compute_attention(self, *args):
        raise NotImplementedError

    def process_output(self, outputs_TNH):
        return outputs_TNH

    def __call__(self, x, is_prefill, kv_cache: KVCache,
...
class DeepseekV3Attention(DeepseekV3BaseAttention):
    ...
    def setup_specific_layers(self, rngs: nnx.Rngs):
    ...
    def compute_q_projection(self, x_q_TD, input_positions):
    ...
    def compute_kv_projection(self, x_SD, input_positions):
    ...
    def compute_attention(self, q_data, kv_data, is_prefill, kv_cache, md):
        q_TNH = q_data
        k_SNH, v_SNH = kv_data
        ...
        in_specs = (self.query_tnh, self.keyvalue_skh, self.keyvalue_skh,
                    P(None, None,
                      ShardingAxisName.ATTN_HEAD), P(), P(), P(), P())
...
class DeepseekV3MLA(DeepseekV3BaseAttention):
    ...
    def compute_attention(self, q_data, kv_data, is_prefill, kv_cache, md):
        ...
        in_specs = (
            self.query_tnh,
            self.query_tnh,  # q, q_rope
            self.keyvalue_skh,
            self.keyvalue_skh,  # k, k_rope
            P(ShardingAxisName.MLP_TENSOR),  # kv_cache
            P(ShardingAxisName.ATTN_DATA),
            P(ShardingAxisName.ATTN_DATA),
            P(ShardingAxisName.ATTN_DATA),
            P(ShardingAxisName.ATTN_DATA)  # Metadata
        )
